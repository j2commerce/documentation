...
Upload the `payment_cardsave.zip` package ZIP file or use the Install from URL option.
...
To help you narrow down the list, you can do a search for **Cardsave Payment**, click the **X,** and it will turn into a green checkmark. It is now enabled and ready for setup.
...
![](/img/cardsave-payment1.webp)

These settings control which Cardsave server endpoint is used. In almost all cases the defaults are correct.

**Payment Processor Domain:** The domain provided by Cardsave for your account. Only change this if Cardsave has given you a different domain.

**Payment Processor Port:** The port for the SOAP connection (Direct mode). Only change this if Cardsave has specified a different port.

### Hash Method

![](/img/cardsave-hash.webp)
...
![](/img/cardsave-status.webp)

These settings control which J2Commerce order status is assigned depending on the payment outcome.

**Authorized Order Status:** Payment was authorized successfully (status code 0 or a duplicate of a successful authorization).

**Declined Order Status:** Payment was declined or an error occurred.

**Pending Order Status:** 3D Secure authentication is in progress (status code 3, Direct mode only).
...
![](/img/cardsave-restrictions.webp)

Use these settings to limit which orders can use Cardsave.

**Minimum Order Total:** Hide Cardsave at checkout if the cart subtotal is below this amount. Set to `0` to disable the minimum.

**Maximum Order Total:** Hide Cardsave at checkout if the cart subtotal exceeds this amount. Set to `-1` to disable the maximum.
