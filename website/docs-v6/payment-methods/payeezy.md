---
title: "Payeezy Payment Plugin"
sidebar_label: "Payeezy (First Data)"
sidebar_position: 2
description: "Accept credit and debit card payments directly via the Payeezy (First Data) API. Step-by-step setup guide for store owners, including credentials, transaction types, sandbox testing, and PCI scope."
---
...
1. Purchase and download the `plg_j2commerce_payment_payeezy.zip` package from the J2Commerce website.
2. Go to **System** -> **Install** -> **Extensions**.
3. Upload the `plg_j2commerce_payment_payeezy.zip` file.
4. The plugin installs and enables automatically.

After installation, navigate to **J2Commerce** -> **Payments** -> **Payment Methods** to configure it.

<!-- SCREENSHOT: J2Commerce Payments menu showing the payment methods list with Payeezy listed -->
...
Go to **J2Commerce** -> **Payments** -> **Payment Methods** and click **Payeezy** (or search for it in the list).

<!-- SCREENSHOT: Payeezy plugin configuration screen in the J2Commerce Payments area -->

### Basic Settings

| Field | Description | Default |
|-------|-------------|---------|
| **Display Name** | The payment method name shown to customers at checkout | `Credit/Debit Card` |
| **Display Image** | Optional logo or card brand image displayed at checkout | _(none)_ |
| **Transaction Type** | How the card is charged — see below | `Purchase (Authorize + Capture)` |
| **Sandbox / Test Mode** | When enabled, transactions go to the Payeezy certification environment. No real charges are made. | Yes (enabled) |
...
<!-- SCREENSHOT: Transaction Type dropdown showing Purchase and Authorize Only options -->
...
| Field | Description | Default |
|-------|-------------|---------|
| **Payment Status** | Order status assigned when a Purchase transaction succeeds | `Confirmed` (status 1) |
| **Authorized Status** | Order status assigned when an Authorize Only transaction succeeds. Only visible in Authorize Only mode. | `Confirmed` (status 1) |
...
|-------|-------------|---------|
| **Surcharge Name** | Label shown to the customer in the order total | `Card Processing Fee` |
| **Surcharge Percent** | Percentage of the order subtotal + shipping to add | `1.5` |
| **Surcharge Fixed** | Fixed amount to add to every card order | `0.30` |
| **Surcharge Tax Class** | Tax profile to apply to the surcharge amount (optional) | _(none)_ |
...
| Field | Description | Default |
|-------|-------------|---------|
| **Show Dashboard Icon** | Add a quick-access icon to the J2Commerce dashboard for this payment method | No |
| **Dashboard Icon Label** | Custom label for the dashboard icon. Only visible when **Show Dashboard Icon** is enabled. | _(plugin name)_ |

### Debug Mode

| Field | Description | Default |
|-------|-------------|---------|
| **Debug Mode** | Log API requests and gateway responses to `payment_payeezy.php` in the Joomla log directory | No |
...
<!-- SCREENSHOT: Payeezy card entry form as it appears to a customer at checkout, showing the four card fields and Place Order button -->
