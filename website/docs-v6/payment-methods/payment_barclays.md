...
2. Upload the `plg_j2commerce_payment_barclays.zip` file.
3. The plugin installs automatically.
4. Go to **J2Commerce** -> **Payments** -> **Payment Methods** and enable **Barclaycard ePDQ**.

<!-- SCREENSHOT: J2Commerce Payments / Payment Methods screen with Barclaycard ePDQ listed and enabled -->
...
<!-- SCREENSHOT: ePDQ Back Office — Technical information page showing the SHA-IN passphrase field -->

## Configure the plugin

Open the plugin by going to **J2Commerce** -> **Payments** -> **Payment Methods** and clicking **Barclaycard ePDQ**.
...
|---|---|---|
| **Hash Algorithm** | SHA-512 | Must match exactly what you selected in the ePDQ Back Office under **Technical information** -> **Global transaction parameters**. SHA-512 is recommended. |
| **Verify SHA-OUT Signature** | Yes | Verifies that every payment notification genuinely comes from Barclaycard. Keep this enabled in production — disable only temporarily for debugging. |
...
| Field | Default | Description |
|---|---|---|
| **Enable Saved Cards** | No | When set to **Yes**, logged-in customers can save their card at checkout and reuse it for future orders without re-entering card details. Requires the Alias feature to be enabled in the ePDQ Back Office. |
| **Integration Mode** | Hosted Payment Page (HPP) | Choose how customers enter card details: **HPP** redirects them to Barclaycard's hosted page; **FlexCheckout** shows a card form on your own checkout page (on-site, SAQ-A, requires the Alias Gateway to be enabled in the Back Office). |
...
| Field | Default | Description |
|---|---|---|
| **Paid Order Status** | Confirmed | Applied when Barclaycard confirms a successful payment (STATUS 5 or STATUS 9) |
| **Pending Order Status** | Pending | Applied when payment is awaiting confirmation (STATUS 51 or STATUS 91) |
| **Failed Order Status** | Failed | Applied when payment is declined, cancelled, or encounters an error |
...
| **Void Order Status** | (none) | Status to apply when a payment is voided (only shown when the above is enabled) |
...
| Field | Default | Description |
|---|---|---|
| **Debug Logging** | No | Writes transaction details to `administrator/logs/plg_j2commerce_payment_barclays.log.php`. Useful when troubleshooting. Disable in production — the log never contains raw card numbers or passphrases. |

<!-- SCREENSHOT: Plugin configuration screen showing the credential fields and Test Mode switcher -->
...
### 3. Set your SHA-OUT passphrase and configure the callback
...
### 4. Enable feedback parameters on redirection URLs (recommended)
...
### 5. Enable the Alias Gateway (if using FlexCheckout or saved cards)
