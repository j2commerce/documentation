...
<!-- SCREENSHOT: Amazon Pay Integration Central showing Public Key ID and key download button -->

## Installing the Plugin

In the Joomla admin, go to **System -> Install -> Extensions**

Upload the `payment_amazonpay.zip` file using the **Upload Package File** tab, or use **Install from URL**.

<!-- SCREENSHOT: Joomla Extensions installer with the upload tab active -->

## Enable the Plugin

Once installed, enable the plugin before configuring it. There are two ways to reach it:

**Option A:** Go to the **J2Commerce** icon at the top right corner **-> Setup -> Payment Methods**

**Option B:** Go to **Components** on the left sidebar **-> J2Commerce -> Dashboard -> Setup -> Payment Methods**

<!-- SCREENSHOT: J2Commerce Payment Methods list showing Amazon Pay with a red X (disabled) -->

Find **Amazon Pay** in the list, click the red **X**, and it turns into a green checkmark. The plugin is now enabled.
...
Click the **Toggle Inline Help** button in the toolbar to show a short description below each field while you configure the plugin.
...
<!-- SCREENSHOT: Amazon Pay Basic Settings tab -->

| Field | Description | Default |
|-------|-------------|---------|
| **Display Name** | The label shown to customers at checkout | `Amazon Pay` |
| **Display Image** | Optional logo shown next to the payment option at checkout | Amazon Pay logo |
| **Checkout Mode** | How the checkout flow works — see options below | Express Checkout |
| **Region** | Your Amazon Pay registration region | United States (NA) |
| **Payment Action** | Whether to charge immediately or authorize first | Authorize and Capture |
| **Product Type** | Express mode only — whether Amazon collects a shipping address | Pay and Ship |
| **Button Color** | Color of the Amazon Pay button displayed at checkout | Gold |
| **Checkout Language** | Language shown on the Amazon-hosted checkout page | English (US) |
| **Restrict PO Boxes** | Express mode only — prevents PO Box shipping addresses | No |
| **Currency Handling** | Whether to charge in the buyer's selected currency or always use your ledger currency | Automatic |
| **Enable Subscriptions** | Allow subscription products to be purchased with Amazon Pay | Yes |
| **Allow Saved Payment Methods** | Let logged-in customers keep their Amazon Pay agreement for renewals and admin charges | Yes |
| **Template Style** | Frontend layout for checkout forms | Blank (auto-detect) |
| **Show Dashboard Icon** | Add an Amazon Pay icon tile to the J2Commerce dashboard | Yes |
| **Debug Logging** | Write API activity to `administrator/logs/payment_amazonpay.php` | No |
...
#### Product Type (Express mode only)
...
<!-- SCREENSHOT: API Credentials tab with Sandbox toggle on -->
...
| Field | Description | Default |
|-------|-------------|---------|
| **Payment Received Status** | Status set after a successful payment capture | Confirmed |
| **Authorized Status** | Status set when payment is authorized but not yet captured | Processing |
| **Failed Status** | Status set when payment fails | Failed |
| **Change Status on Cancel** | Automatically update status when a charge is canceled | No |
| **Cancel Order Status** | Status to apply after a charge cancellation (shown when above is Yes) | — |
| **Change Status on Refund** | Automatically update status when a refund is processed | No |
...
| Field | Description | Default |
|-------|-------------|---------|
| **Surcharge Name** | Label shown to customers (e.g., "Processing Fee") | Blank |
| **Surcharge Percent** | Percentage of the order subtotal to add | `0` |
| **Surcharge Fixed** | Fixed amount to add in your store currency | `0` |
...
| Field | Description | Default |
|-------|-------------|---------|
| **Geo-Zone Restriction** | Only show Amazon Pay to customers in a specific geo-zone. Leave empty for all customers | None |
| **Minimum Subtotal** | Hide Amazon Pay if the cart subtotal is below this amount. Leave at `0` for no minimum | `0` |
| **Maximum Subtotal** | Hide Amazon Pay if the cart subtotal exceeds this amount. Set to `-1` for no maximum | `-1` |
...
When **Enable Subscriptions** is set to **Yes**, customers can use Amazon Pay for subscription products (requires the app_subscriptionproduct add-on).
...
The IPN endpoint URL is:
`https://[your-site.com]/index.php?option=com_ajax&plugin=payment_amazonpay&group=j2commerce&task=ipn&format=raw`
