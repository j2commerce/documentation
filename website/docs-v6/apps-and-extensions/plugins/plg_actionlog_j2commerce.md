---
title: "Action Log Plugin"
sidebar_label: "Action Log"
sidebar_position: 1
description: "Track basket, checkout, and order activity in your J2Commerce store using the built-in Action Log plugin."
---
...
2. Search for **J2Commerce - Action Log**.
3. Click the checkbox next to it and then click **Enable** in the toolbar.

<!-- SCREENSHOT: System > Manage > Extensions filtered to show "J2Commerce - Action Log" with the Enable button highlighted -->
...
Go to **System** -> **Manage** -> **Extensions**, search for **J2Commerce - Action Log**, and click the plugin name.

<!-- SCREENSHOT: Plugin configuration screen showing the four parameter fields -->

### Email Notification Level

| Setting | Description | Default |
|---------|-------------|---------|
| **Email Notification Level** | The minimum event priority that triggers an email alert. Events with a lower priority than the selected level are logged silently without sending email. | Warning |
...
| Setting | Description | Default |
|---------|-------------|---------|
| **Notification User Groups** | The Joomla user groups whose members receive email alerts. You can select multiple groups. | (none) |
...
| Setting | Description | Default |
|---------|-------------|---------|
| **Log Basket Events** | Records when shoppers add items to the basket, remove items, update quantities, or clear the entire basket. | Yes |
| **Log Checkout Events** | Records each step of the checkout funnel — start, login, billing address, shipping address, payment step, and order review. | Yes |
| **Log Order Events** | Records when a payment is completed (with its result) and when an administrator changes an order status. | Yes |
...
4. Checks whether the event's priority meets the configured notification threshold and, if so, sends email to the members of the selected user groups.

Entries appear immediately in **Users** -> **Action Logs**. Each row shows the date and time, the username (linked to their user record), and a description of what happened.

<!-- SCREENSHOT: Users > Action Logs filtered to show J2Commerce entries, displaying basket and order events -->
...
1. Go to **System** -> **Manage** -> **Extensions** and confirm the plugin status shows as **Enabled**.
2. Make sure the relevant event category toggle (**Log Basket Events**, **Log Checkout Events**, or **Log Order Events**) is set to **Yes**.
3. Navigate to **Users** -> **Action Logs** and check that the log is not filtered to a date range that excludes today.
...
- [Action Logs (Joomla documentation)](https://docs.joomla.org/action_logs) — how to view and manage Joomla action logs
- [Order Statuses](../../localisation/order-statuses.md) — configure the status names that appear in log entries
- [Orders](../../sales/orders.md) — manage and review orders in J2Commerce
