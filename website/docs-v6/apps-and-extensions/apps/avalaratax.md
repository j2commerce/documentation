...
The **AvalaraTax** is a separate add-on available from the [J2Commerce Extensions Store](https://www.j2commerce.com). It is not included with the core J2Commerce 6 component.

**Step 1:** Go to [www.j2commerce.com](https://www.j2commerce.com) -> **Apps**.

**Step 2:** Locate the **AvalaraTax** app, click **View Details**, then **Add to Cart** -> **Checkout**.
...
Upload the plugin `plg_j2commerce_app_avalaratax.zip` file or use the Install from URL option.
...
To help you narrow down the list, you can do a search for **AvalaraTax**, click the **X,** and it will turn into a green checkmark. It is now enabled and ready for setup.

![](/img/avalara_enable.webp)
...
![](/img/avalara_toggle.webp)

### Avalara Credentials tab

![](/img/avalara_api.webp)

**Environment:** Sandbox for testing, Production for live transactions.

While you are still setting up and testing, keep **Environment** (at the top of the **Plugin** tab) set to **Sandbox**. Sandbox transactions don't create real tax liability and won't appear in your production Avalara account. Switch to **Production** only after you have completed all testing
...
![](/img/avalara_calculation.webp)

**Tax Profile:** The J2Commerce tax profile whose line items Avalara calculates tax for. Create this profile yourself in **Localisation** -> **Tax Profiles** first — the plugin does not create one automatically.

**Disable Document Recording:** When Yes, checkout still estimates tax but no document is ever created or committed in Avalara — useful if another connector already records documents there.
...
**Enable Fallback Rate:** If the Avalara API is unreachable, apply a flat percentage instead of failing the rate lookup.

**Fallback Rate:** The percentage rate applied when fallback is active (0–100). Only shown when Enable Fallback Rate is Yes.

**Colorado / Minnesota Retail Delivery Fee:** When Yes, adds the required Colorado/Minnesota retail delivery fee to eligible orders as its own order fee. Avalara computes the exact amount — the plugin never estimates it.
...
![](/img/avalara_tax.webp)

**Default Tax Code:** Fallback tax code used when a product has no product-level or category-level mapping. `P0000000` covers general tangible personal property.

**Shipping Tax Code:** Tax code applied to the synthetic shipping line on every transaction.

**Customer Code Source:** How to identify the customer in Avalara: by email address, Joomla user ID, or order ID.

**Tax Display Name:** The label shown to customers for the calculated tax line in cart and checkout totals.

### Exemptions

![](/img/avalara_exemption.webp)
...
**Send Tax/VAT Number:** When Yes, the customer's tax/VAT number (from the order's billing or shipping address) is sent to Avalara as the business identification number.

### Address Validation

![](/img/avalara_address.webp)

**Address Validation Mode:** Disabled skips validation entirely. Warn shows a notice but lets checkout continue. Require blocks the shipping step until a resolvable address is entered.

**Validation Countries:** Countries checked against Avalara's address resolution. Avalara only supports the United States and Canada — other countries always skip validation regardless of this setting.

### Shipping Origin tab

![](/img/avalara_shipping.webp)
...
**Log API Requests:** When Yes, the full sanitized request/response and round-trip time for every Avalara API call is recorded to a database log, viewable from the AvaTax dashboard's API Log screen.
...
## Create and assign an Avalara tax profile
...
2. Open the AvalaraTax App and go to the **Calculation** tab.
3. Set **Tax Profile** to the profile you just created.
4. Click **Save**.

![](/img/avalara_tax1.webp)
...
### Place a test order and verify tax
...
1. Set the task to run on a recurring interval — every 5 minutes is a reasonable starting point for most stores.
2. Save the task.
...
- Go to **J2Commerce** -> **Apps** -> **Avalara AvaTax** to open the **dashboard**.

![](/img/avalara_dashboard.webp)

- Click the **Tax Code Mapping** under the **quick link** section

![](/img/avalara_tax-code.webp)

- Add a row for each category you want to map, and enter its Avalara tax code.

![](/img/avalara_tax-code1.webp)
...
To assign the Avalara tax profile to many products at once instead of opening each one individually:

- From the AvaTax **dashboard**, click the **Product Tax Profiles** under the **quick link** section.

![](/img/avalara_tax-code-products.webp)
...
![](/img/avalara_tax-code-products-bulk.webp)
...
- Go to **J2Commerce** **->** **Apps** **->** **Avalara AvaTax** **-> Dashboard**

- The **Sync Queue** card on the Dashboard shows live counts for Pending, Processing, Failed, and Dead items, plus the ten most recent failures with their error messages.

![](/img/avalara_sync.webp)

- Click the **View Queue Log** quick link (or the **AvaTax Sync Queue** menu item) for the full list.

![](/img/avalara_view.webp)
