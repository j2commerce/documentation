...
Install using the standard Joomla extension installer.

In the Joomla admin, go to **System** -> **Install** -> **Extensions**

Upload the plugin ZIP file or use the **Install from URL** option.

<!-- SCREENSHOT: Joomla Extensions installer screen with the Square Integration ZIP being uploaded -->
![Square Integration install screen](/img/square-integration-install.webp)
...
<!-- SCREENSHOT: J2Commerce Apps list highlighting the Square Integration row with its enable toggle -->
![J2Commerce Apps list](/img/square-integration-apps-list.webp)

Find **Square Integration** in the list, click the status icon (it will turn from a red X to a green checkmark). The app is now enabled and ready to configure.
...
![Connection tab fields](/img/square-integration-connection-tab.webp)
...
![Connected status after OAuth](/img/square-integration-connected.webp)
...
|---------|-------------|---------|
| **Environment** | Choose **Sandbox** while testing or **Production** when live | Sandbox |
...
| **Inventory Aggregation** | **Sum** — adds up stock counts across all selected locations. **Primary** — uses only the first selected location's count | Sum |
...
| **Category Mapping Mode** | **Tag** — maps each Square category to a J2Commerce tag. **Category** — maps each Square category to a Joomla content category | Tag |
| **Option Field Type** | How variant attributes (size, color, etc.) are presented to shoppers. **Select dropdown**, **Radio buttons**, or **Color swatches** | Select dropdown |
| **Delete Action** | What happens in J2Commerce when an item is deleted in Square. **Unpublish** keeps the product but hides it. **Move to Trash** removes it from the active list. **Ignore** leaves the J2Commerce product untouched | Unpublish |
...
| Setting | Description | Default |
|---------|-------------|---------|
| **Pricing Mode** | **Direct** — use Square price as-is. **Markup** — add a percentage or fixed amount on top of the Square price | Direct |
| **Markup Type** | Shown only when Markup mode is selected. **Percent** or **Fixed amount** | — |
| **Markup Value** | The markup amount to add. Enter a whole number or decimal | 0 |
...
| Setting | Description | Default |
|---------|-------------|---------|
| **Enable Webhooks** | When **Yes**, Square sends real-time push notifications to J2Commerce when products or inventory change. Requires an HTTPS site reachable from the public internet | Yes |
| **Cron: Catalog Interval** | How often (in minutes) the scheduled task checks Square for catalog changes. Minimum 15 minutes | 60 |
| **Cron: Inventory Interval** | How often (in minutes) the scheduled task refreshes inventory counts. Minimum 5 minutes | 30 |
| **Catalog Cache TTL** | How long (in minutes) Square catalog data is cached locally before a fresh fetch is requested. Minimum 60 minutes | 1440 |
...
| Setting | Description | Default |
|---------|-------------|---------|
| **Error Threshold** | Number of consecutive sync errors before J2Commerce raises an alert in the admin | 10 |
| **Sync Log Retention Days** | How many days of sync activity logs to keep before automatic cleanup. Minimum 7 days | 30 |
| **Debug Mode** | When **Yes**, verbose logging is written to the J2Commerce log file. Disable on production | No |
| **OAuth Debug Page** | When **Yes**, clicking Connect to Square shows a debug card instead of immediately redirecting. The card displays the authorize URL, Application ID, OAuth state, and Redirect URI for inspection. Toggle back to **No** to restore the normal flow | No |
