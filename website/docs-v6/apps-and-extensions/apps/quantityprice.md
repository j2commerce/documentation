...
The **Quantity Dropdown** app is a separate add-on available from the [J2Commerce Extensions Store](https://www.j2commerce.com). It is not included with the core J2Commerce 6 component.
...
You can install this **Quantity Dropdown** App using the Joomla installer. The following steps help you with a successful installation.

In the Joomla admin, go to **System -> Install -> Extensions**

Upload the `app_quantityprice.zip` file or use the Install from URL option.
...
**Enable for all products:** Turn on the dropdown for every product in the store. You can still override this per product.

**Quantity box type:** Choose **Dropdown** to replace the number input with a select list, or **Default Input** to keep the standard number field.

**Pre-defined quantities (comma-separated):** The quantities shown in the dropdown. Enter values separated by commas — for example `1,5,10,25,50`. Used when **Quantity box type** is set to **Dropdown** and the factor option is off.

**Show placeholder option:** Adds a **--Select--** option at the top of the dropdown. The customer must pick a value before adding to cart.

**Restrict to user groups:** Limit the dropdown to selected Joomla user groups. Leave empty to show to all visitors.
...
## Configure Factor Settings (Optional)
...
**Use factor to generate quantity list:** Enable factor-based generation. When on, the **Pre-defined quantities** field is ignored.

**Quantity factor:** The step between each option in the dropdown.

**Maximum quantity limit:** The largest quantity in the dropdown. Options are generated from the factor up to this value.
...
**Multiply price by quantity:** When enabled, the displayed product price updates to show the total (unit price multiplied by the selected quantity) as the customer changes the dropdown.

**Enable quantity dropdown in cart:** Show the quantity dropdown on the cart page so customers can adjust quantities using the same dropdown instead of a free-text number field.
...
**Pre-text:** Text to show before each quantity value in the dropdown option label.

**Post-text:** Text to show after each quantity value in the dropdown option label.

**Debug Mode:** Write detailed logs to the Joomla log directory for troubleshooting. Disable this in a live store.
