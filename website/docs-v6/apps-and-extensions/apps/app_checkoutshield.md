...
The **Checkout Shield** app is a separate add-on available from the [J2Commerce Extensions Store](https://www.j2commerce.com). It is not included with the core J2Commerce 6 component.

**Step 1:** Go to the [J2Commerce website](https://www.j2commerce.com/) -> **Apps**.

**Step 2:** Locate the **Checkout Shield** app -> click **View Details** -> **Add to cart** -> **Checkout**.
...
Upload `plg_j2commerce_app_checkoutshield.zip`, then upload `plg_task_j2commerce_checkoutshield.zip`.
...
Look for **Checkout Shield**, click the **X,** and it will turn into a green checkmark. It is now enabled and ready for setup.

Click the plugin name to open its settings — this is where every option in this guide lives.

The dashboard lives in a different place: **J2Commerce** -> **Analytics** -> **Checkout Shield**.
...
![](/img/checkout_shield_toggle.webp)

## The Monitor-First Rollout (Read This Before Anything Else)

![](/img/checkout_shield_monitor.webp)
...
Everything Checkout Shield sees lives in one place: The Dashboard. There are two ways to get to the Checkout Shield Dashboard.&#x20;

**Option A:** J2Commerce **->** Apps **->** Checkout Shield

**Option B:** J2Commerce **->** Setup **->** Checkout Shield

*The image below shows the dashboard in the Apps plugin*

![](/img/checkout_shield_dashboard1.webp)
...
![](/img/checkout_shield_dashboard.webp)
...
![](/img/checkout_shield_attack.webp)
...
![](/img/checkout_shield_attempts.webp)

Every checkout request Checkout Shield evaluates gets a row here — go to **Checkout Shield -> Dashboard -> Quicklinks ->** **Attempts Log**, or click the **View all** link under "Recent attempts" on the dashboard.
...
![](/img/checkout_shield_blocklist.webp)

Go to **Checkout Shield -> Dashboard -> Quicklinks ->** **Blocked IPs** to see every IP currently on the blocklist, whether it got there automatically (repeat offenses) or you put it there by hand.
...
![](/img/checkout_shield_allow-list.webp)

Go to **Checkout Shield -> Dashboard -> Quicklinks -> Allowlist** to manage entries that **always bypass every check**, no matter what — including Lockdown Mode.
...
Go to **Checkout Shield -> Dashboard -> Enable Lockdown**

![](/img/checkout_shield_lockdown1.webp)

Lockdown Mode is a **one-click emergency switch**, available from the toolbar on Checkout Shield Dashboard page. Turn it on when you are watching a real attack unfold and need to slam the door immediately, without waiting to retune individual settings.
...
![](/img/checkout_shield_lockdown2.webp)
...
![](/img/checkout_shield_notification.webp)
...
![](/img/checkout_shield_notification1.webp)
...
| **Project Honey Pot (http\:BL&#xA;)** | Yes (free)                        | **Off**                                    | DNS-based lookup, cached, \~1-second timeout.                                                                                               |
...
![](/img/checkout_shield_system.png)
