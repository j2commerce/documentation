---
title: "AtoShip Multi-Carrier Shipping"
sidebar_label: "AtoShip Shipping"
sidebar_position: 30
description: "Connect your J2Commerce store to AtoShip for real-time shipping rates from USPS, UPS, FedEx, and DHL, plus label printing and package tracking."
---
...
1. Purchase and download the `shipping_atoship.zip` package from the J2Commerce website.
2. In your Joomla Administrator, go to **System** -> **Install** -> **Extensions**.
3. Upload the `shipping_atoship.zip` package file.
4. The plugin installs and enables automatically.
5. Navigate to **J2Commerce** -> **Shipping** -> **Shipping Methods** to configure it.

<!-- SCREENSHOT: System > Install > Extensions upload screen with shipping_atoship.zip -->
...
AtoShip provides two types of API keys:

| Key Type | Format | Purpose |
|----------|--------|---------|
| **Test (Sandbox)** | Starts with `ak_test_` | For testing without real charges |
| **Live (Production)** | Starts with `ak_live_` | For real shipments and label purchases |

To find your API keys:

1. In the AtoShip dashboard, go to **Settings** (or **Account**) -> **API Keys**.
2. Copy both your **Test API Key** and **Live API Key**.
3. Keep these keys safe -- you will need them in the next step.

<!-- SCREENSHOT: AtoShip dashboard showing API Keys section -->

## Step 2: Connect AtoShip to J2Commerce

1. In your Joomla Administrator, go to **J2Commerce** -> **Shipping** -> **Shipping Methods**.
2. Find **AtoShip Multi-Carrier Shipping** in the list and click its name to open the configuration.

<!-- SCREENSHOT: J2Commerce Shipping Methods list showing AtoShip plugin -->

### API Credentials

| Setting | Description | Recommended |
|---------|-------------|-------------|
| **Sandbox Mode** | When set to **Yes**, the plugin uses your test API key and no real charges are made. Set to **No** for live shipments. | Start with **Yes** |
| **Sandbox API Key** | Your test API key (starts with `ak_test_`). Only visible when Sandbox Mode is Yes. | Paste your test key |
| **Live API Key** | Your production API key (starts with `ak_live_`). Only visible when Sandbox Mode is No. | Paste your live key |

3. Leave **Sandbox Mode** set to **Yes** for initial setup.
4. Paste your **Test API Key** into the **Sandbox API Key** field.
...
## Step 3: Set Your Ship-From Address
...
<!-- SCREENSHOT: Ship-From Address fields in the plugin configuration -->

| Field | Description | Example |
|-------|-------------|---------|
| **Origin Address** | Your street address | `123 Main Street` |
| **Origin City** | Your city | `Los Angeles` |
| **Origin State** | Your state or province code (2-letter abbreviation) | `CA` |
| **Origin Postal Code** | Your ZIP or postal code (**required**) | `90001` |
| **Origin Country** | Your 2-letter country code | `US` |

1. Fill in all five address fields with your business or warehouse address.
2. The **Origin Postal Code** is required -- the plugin will not calculate rates without it.
3. Click **Save**.
...
<!-- SCREENSHOT: Allowed Carriers section showing four carrier cards with checkboxes -->

1. Click the checkbox on each carrier card you want to enable.
2. You must have an active carrier account with AtoShip for each carrier you select. Check your AtoShip dashboard under **Carrier Accounts** to see which carriers are connected.
3. Click **Save**.

| Carrier | Best For |
|---------|----------|
| **USPS** | Domestic US shipments, lightweight packages, affordable rates |
| **UPS** | Domestic and international, heavier packages, reliable tracking |
| **FedEx** | Express and overnight, domestic and international |
| **DHL** | International shipments, global coverage |

:::tip
Start with one or two carriers to keep the checkout simple. You can always add more later.
:::

## Step 5: Discover and Select Services
...
<!-- SCREENSHOT: Discover Available Services button and test destination fields -->

4. The plugin queries the AtoShip API and displays all available services grouped by carrier.

<!-- SCREENSHOT: Service discovery results showing services grouped by carrier with multi-select -->
...
|---------|-------------|---------|
| **Show Delivery Time** | When set to **Yes**, estimated delivery days appear below each shipping option at checkout (e.g., "3 Business Days"). | Yes |

## Step 6: Set Package Defaults
...
<!-- SCREENSHOT: Package Defaults section showing weight and dimension fields -->

| Setting | Description | Default |
|---------|-------------|---------|
| **Weight Unit** | The unit for all weight values. Choose **LBS** (pounds) or **KGS** (kilograms). | LBS |
| **Dimension Unit** | The unit for all dimension values. Choose **IN** (inches) or **CM** (centimeters). | IN |
| **Default Weight** | Fallback weight per item when a product has no weight set. | `1` |
...
## Step 7: Configure Rate Adjustments (Optional)

These settings let you adjust the rates returned by AtoShip before they are shown to customers.

<!-- SCREENSHOT: Rate Adjustments section showing handling fee and tax profile fields -->

| Setting | Description | Default |
|---------|-------------|---------|
| **Handling Fee** | An additional fee added to each shipping rate. Use this to cover packaging materials or labor. | `0` |
| **Handling Fee Type** | How the handling fee is calculated: **Flat** (fixed amount) or **Percent** (percentage of the shipping rate). | Flat |
| **Tax Profile** | If shipping is taxable in your jurisdiction, select a J2Commerce tax profile. Leave as **None** to not charge tax on shipping. | None |
| **Geo Zone** | Restrict this shipping method to customers in a specific geographic zone. Leave as **All** to show rates to all customers. | All |
| **Subtotal Minimum** | Only show AtoShip rates when the cart subtotal is at or above this amount. Set to `0` to disable. | `0` |
| **Subtotal Maximum** | Only show AtoShip rates when the cart subtotal is at or below this amount. Set to `0` to disable. | `0` |

**Examples:**

- To add a $2.50 packing fee to every shipment, set **Handling Fee** to `2.50` and **Handling Fee Type** to **Flat**.
- To add a 10% markup on shipping rates, set **Handling Fee** to `10` and **Handling Fee Type** to **Percent**.
- To only offer AtoShip for orders over $25, set **Subtotal Minimum** to `25`.

## Step 8: Configure Label and Tracking Settings

These settings control what happens when you create shipping labels and how tracking updates affect your orders.

<!-- SCREENSHOT: Label & Tracking section showing order status and webhook fields -->

| Setting | Description | Default |
|---------|-------------|---------|
| **Shipped Order Status** | The order status to set automatically when a shipping label is created. | Shipped |
| **Delivered Order Status** | The order status to set automatically when tracking shows the package was delivered. | Delivered |
| **Notify Customer on Delivery** | Send an email to the customer when the package tracking status changes to "Delivered". | No |
| **Webhook Secret** | The HMAC-SHA256 secret key from your AtoShip dashboard. Required for automatic tracking updates via webhooks. | (empty) |
| **Tracking Poll Limit** | Maximum number of shipments to check for tracking updates during each cron job run. | `50` |
...
| Setting | Description | Default |
|---------|-------------|---------|
| **Debug** | When enabled, all AtoShip API requests and responses are logged to `administrator/logs/shipping_atoship.php`. Useful for troubleshooting rate or label issues. | No |

Turn this on temporarily if you experience problems. Remember to turn it off in production to avoid growing log files.

## Step 10: Test at Checkout
...
<!-- SCREENSHOT: Checkout shipping step showing AtoShip rates with carrier logos and delivery estimates -->
...
<!-- SCREENSHOT: Insufficient Balance modal showing current balance and estimated cost -->
...
<!-- SCREENSHOT: Order view showing Create Shipping Label button in the toolbar -->

2. A modal window opens with the label creation form:

<!-- SCREENSHOT: Label creation modal showing carrier, service, package details, and insurance fields -->
...
3. Review and adjust the package details if needed. The carrier and service are pre-filled from the customer's checkout selection.
4. Toggle **Include Insurance** if you want to insure the shipment.
...
<!-- SCREENSHOT: Order toolbar showing View Label and Print Label buttons after label creation -->
...
<!-- SCREENSHOT: Shipping & Tracking panel showing a label with tracking information -->
...
<!-- SCREENSHOT: Orders list with multiple orders selected and Create AtoShip Labels button highlighted -->
...
### Step 2: Configure AtoShip Webhooks
...
<!-- SCREENSHOT: AtoShip dashboard webhook configuration page -->

### Step 3: Add the Webhook Secret to J2Commerce
...
- [Shipping Methods](../shipping/index.md) - Overview of all shipping options in J2Commerce
- [Orders](../orders/index.md) - Managing orders and order statuses
- [Tax Profiles](../taxes/index.md) - Setting up tax profiles for shipping charges
- [Configuration](../configuration/index.md) - General store configuration
