---
title: "J2Commerce Migrator"
sidebar_label: "J2Commerce Migrator"
sidebar_position: 1
description: "Step-by-step guide for migrating your J2Store v4 store data to J2Commerce 6 using the J2Commerce Migrator component."
---
...
## Get the Migrator {#get-the-migrator}

The J2Commerce Migrator is a free separate download — it does not ship inside the core J2Commerce package.

**Step 1:** Log in to your account at [j2commerce.com](https://www.j2commerce.com/) and open your customer downloads area.

**Step 2:** Search for **J2Commerce Migrator** and click **View Files -> Download Now** to download `pkg_j2commercemigrator_*.zip`.

## Install the Migrator {#install-the-migrator}
...
**Step 3:** Joomla installs the Migrator component and the J2Store 4 source connector together. You will see a success message when the installation is complete.
...
|---------|-------------|---------|
| **Default Batch Size** | Number of database rows processed per batch. Lower values use less memory on shared hosting. | 200 |
| **Default Conflict Mode** | What to do when a row already exists in the J2Commerce database. See conflict modes below. | Skip |
| **Enable Migration Log** | Write detailed activity to `administrator/logs/com_j2commercemigrator.log`. | Yes |
| **Enable SSH Tunnel Mode** | Show the Mode C (SSH tunnel) connection option in the wizard. Disabled by default to keep the connection step simple. | No |
| **Add to J2Commerce Menu** | Show a Migrator shortcut in the J2Commerce admin navigation bar. | Yes |
| **Trigger Akeeba Backup** | Automatically trigger a fresh Akeeba backup before any data is migrated. Only visible when Akeeba Backup is installed. | Yes |
| **Akeeba Backup Profile** | Which Akeeba profile to use for the pre-migration backup. | 1 |
...
**Tools** provides targeted re-sync operations that you can run after an initial migration — for example, to pull in new orders that arrived while you were testing, without re-running a full migration.
...
**Uninstall J2Store v4** — Removes the com_j2store component and all associated J2Store files from this Joomla installation. The J2Store database tables are not deleted by this step. If you uninstall J2Store before migration is complete, the Migrator will warn you about tables that have not been fully migrated.
...
When you click **Start Migration** and work through the wizard, here is what happens at each stage:
