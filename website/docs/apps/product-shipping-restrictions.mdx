...
If the "Prevent customer from checking out if shipping method was not chosen" (Configuration > Cart > (towards the bottom) )must be set to Yes in order to prevent the restricted order from shipping.
