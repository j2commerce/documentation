...
function getAlgoliaIndexNameFromPathname(pathname, fallbackIndexName) {
  const normalizedPathname = String(pathname || '/').toLowerCase();

  // Match more specific prefixes first.
  if (normalizedPathname.startsWith('/developer')) {
    return 'j2c_developer_v6';
  }
  if (normalizedPathname.startsWith('/v6')) {
    return 'j2c_docs_v6';
  }

  return fallbackIndexName || 'j2c_docs';
}
...
export default function SearchBar() {
  ...
  const resolvedIndexName = getAlgoliaIndexNameFromPathname(
    location.pathname,
    algoliaConfig.indexName,
  );

  return <DocSearch {...algoliaConfig} indexName={resolvedIndexName} />;
}
